...
impl Png {
    pub fn build(bytes: &Vec<u8>) -> Result<Png, ()> {
        ...
        let mut length = usize::try_from(Self::as_u32(&bytes[0..4])).unwrap();

        while chunk_cursor + length + 13 < bytes.len() {
            length = usize::try_from(Self::as_u32(&bytes[chunk_cursor..chunk_cursor + 4])).unwrap();
            let chunk_type = Self::as_u32(&bytes[chunk_cursor + 4..chunk_cursor + 8]);
            let chunk_data = bytes[chunk_cursor + 8..chunk_cursor + length].to_vec();
            let crc = Self::as_u32(&bytes[chunk_cursor + length..chunk_cursor + length + 4]);
